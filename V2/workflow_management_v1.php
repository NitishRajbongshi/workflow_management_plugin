...
include_once __DIR__ . '/backend/WorkflowInstance.php';
include_once __DIR__ . '/backend/InstanceController.php';
 ...
 * Create an instance
 */
function create_instance(WP_REST_Request $request)
{
    $workflow_name = $request['workflow'];
    $employee_id = $request['employee_id'];
    $instance_name = $request['instance_name'];
    $instance_desc = $request['instance_desc'];

    $obj = new WorkflowInstance();
    $obj->set_workflow($workflow_name);
    $obj->set_employee_id($employee_id);
    $obj->set_instance_values($instance_name, $instance_desc);
    if ($obj->create()) {
        return array(
            "status" => "success",
 ...
 * Get an instance
 */
function get_single_instance(WP_REST_Request $request)
{
    $id = $request['id'];
    $obj = new WorkflowInstance();
    if ($obj->load_instance($id) == false) {
        return array(
            "status" => "Failed",
            "message" => "Workflow instance not found",
        );
    } else {
        $res = $obj->load_instance($id);
        return $res;
    }
}

/**
 * Get all the instance for a particular person
 */
function get_all_instance(WP_REST_Request $request)
{
    $employee_id = $request['employee_id'];
    $obj = new WorkflowInstance();
    $result = $obj->load_all_instance($employee_id);
    if ($result == false) {
        return array(
            'status' => 'failed',
            'message' => 'You do not have any request yet'
        );
    } else
        return $result;
}
...
/**
 * Show all the request to the correct handler person
 */
function show_instance_to_handler_person(WP_REST_Request $request)
{
    $employee_id = $request['employee_id'];
    $obj = new WorkflowInstance();
    $obj->set_employee_id($employee_id);
    $response = $obj->show_single_instance();
    return $response;
}
...
function user_login(WP_REST_Request $request)
{
    $email = $request['email'];
    $password = $request['password'];
    $login_obj = new Authentication();
    $employee_id = $login_obj->get_employee_id($email);
    $info = $login_obj->get_employee_details($employee_id);
